...
import {Button, Card, Col, Form, Input, Layout, Modal, Popconfirm, Row, Space, Typography,} from 'antd';
import {DeleteOutlined, LeftOutlined, PlusOutlined, ReloadOutlined, RightOutlined,} from '@ant-design/icons';
				...
				<ProCard bordered bodyStyle={{padding: 0, margin: 0}}>
					<ProCard>
						<Space>
							...
							<Title level={3} style={{marginBottom: 0, paddingBottom: 0}}>{project?.name}</Title>
						</Space>
					</ProCard>
			...
			<Content style={{padding: '16px'}}>
				{project && (
					<>
						<Row gutter={[16, 16]}>
							<Col span={8}>
								<Card loading={isLoading} title="To-Do" style={{height: '100%'}}>
									{tasks.todo.map((task) => (
										<Card
											key={task.id}
											title={task.title}
							...
							<Col span={8}>
								<Card loading={isLoading} title="In Progress" style={{height: '100%'}}>
							...
							<Col span={8}>
								<Card loading={isLoading} title="Done" style={{height: '100%'}}>
									{tasks.done.map((task) => (
										<Card
											key={task.id}
											title={task.title}
